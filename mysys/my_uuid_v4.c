...
static my_bool my_uuid_v4_inited= 0;
...
int is_random_bits_generation_successful(unsigned char *rand_var, int size)
{
  return my_random_bytes(rand_var, size) != MY_AES_OK;
}

/**
  Init structures needed for my_uuid_v4
*/
void my_uuid_v4_init()
{
  if (my_uuid_v4_inited)
    return;
  my_uuid_v4_inited= 1;
}
...
*/
void my_uuid_v4(uchar *to)
{
  uint64 random_a, random_c, random_c_and_variant;
  uint16 random_b, random_b_and_version;

  DBUG_ASSERT(my_uuid_v4_inited);
                                           ...
                                           sizeof(random_c)))
  {
    my_printf_error(CR_UNKNOWN_ERROR,
                "Failed to generate the random section of UUIDv4\n",
                MYF(ME_FATAL|ME_ERROR_LOG));
  }

  random_b_and_version= (uint16)
    ((random_b & UUID_VERSION_MASK) | UUID_VERSION);
  random_c_and_variant= (uint64) ((random_c & UUID_VARIANT_MASK) | UUID_VARIANT);
